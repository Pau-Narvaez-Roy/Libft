...
int main(void)
{
    printf("%d\n", ft_isalpha('a'));    // Output: 1
    printf("%d\n", ft_isdigit('5'));    // Output: 1
    printf("%d\n", ft_isalnum('a'));    // Output: 1
    return (0);
}
```
...
int main(void)
{
    size_t len = ft_strlen("hello");     // len = 5
    char *dup = ft_strdup("hello");      // dup = "hello"
    char *joined = ft_strjoin("hello", " world");  // "hello world"
    char **split = ft_split("a,b,c", ',');  // ["a", "b", "c"]
    
    free(dup);
    free(joined);
    // Liberar split correctamente...
    return (0);
}
```
...
int main(void)
{
    ...
    t_list *last = ft_lstlast(head);  // "third"
...
#### Escritura con File Descriptors
...
int main(void)
{
    ft_putchar_fd('A', 1);           // Output: A (a stdout)
    ft_putstr_fd("Hello", 1);        // Output: Hello
    ft_putendl_fd("World", 1);       // Output: World\n
    ft_putnbr_fd(42, 1);             // Output: 42
    return (0);
}
```
